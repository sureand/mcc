...
static struct Map *do_make_map(struct Map *parent, int size) {
...
static void maybe_rehash(struct Map *m) {
    if (!m->key) {
...
static void *map_get_nostack(struct Map *m, char *key) {
    if (!m->key)
        return NULL;
    int mask = m->size - 1;
    int i = hash(key) & mask;
